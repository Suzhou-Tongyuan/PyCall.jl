...
function pysym_impl(::Val{funcname}) where funcname
    Libdl.dlsym(ref_libpyhandle[], funcname)
end

function pyglobal_impl(::Val{name}) where name
    Libdl.dlsym(ref_libpyhandle[], name)
end

function pyglobalobj_impl(::Val{name}) where name
    reinterpret(Ptr{PyObject_struct}, Libdl.dlsym(ref_libpyhandle[],  name))
end

function pyglobalobjptr_impl(::Val{name}) where name
    unsafe_load(reinterpret(Ptr{Ptr{PyObject_struct}}, Libdl.dlsym(ref_libpyhandle[],  name)))
end

macro pysym(funcname)
    :($pysym_impl($Val($(esc(funcname)))))
end

macro pyglobal(name)
    :($pyglobal_impl($Val($(esc(name)))))
end

macro pyglobalobj(name)
    :($pyglobalobj_impl($Val($(esc(name)))))
end

macro pyglobalobjptr(name)
    :($pyglobalobjptr_impl($Val($(esc(name)))))
end
